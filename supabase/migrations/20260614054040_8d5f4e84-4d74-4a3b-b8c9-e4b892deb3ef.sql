...
CREATE POLICY "eligibility_results_select_own" ON public.eligibility_results FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "eligibility_results_insert_own" ON public.eligibility_results FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "eligibility_results_update_own" ON public.eligibility_results FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "eligibility_results_delete_own" ON public.eligibility_results FOR DELETE TO authenticated USING (auth.uid() = user_id);
...
CREATE POLICY "cv_optimizations_select_own" ON public.cv_optimizations FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "cv_optimizations_insert_own" ON public.cv_optimizations FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "cv_optimizations_update_own" ON public.cv_optimizations FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "cv_optimizations_delete_own" ON public.cv_optimizations FOR DELETE TO authenticated USING (auth.uid() = user_id);
...
CREATE POLICY "generated_sops_select_own" ON public.generated_sops FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "generated_sops_insert_own" ON public.generated_sops FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "generated_sops_update_own" ON public.generated_sops FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "generated_sops_delete_own" ON public.generated_sops FOR DELETE TO authenticated USING (auth.uid() = user_id);
CREATE TRIGGER generated_sops_set_updated_at BEFORE UPDATE ON public.generated_sops FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE POLICY "sent_reminders_select_own" ON public.sent_reminders FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "sent_reminders_insert_own" ON public.sent_reminders FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "sent_reminders_delete_own" ON public.sent_reminders FOR DELETE TO authenticated USING (auth.uid() = user_id);

CREATE POLICY "Allow authenticated upload to documents" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'documents' AND auth.uid()::text = split_part(name, '/', 1));
CREATE POLICY "Allow authenticated read own documents" ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'documents' AND auth.uid()::text = split_part(name, '/', 1));
CREATE POLICY "Allow authenticated delete own documents" ON storage.objects FOR DELETE TO authenticated
  USING (bucket_id = 'documents' AND auth.uid()::text = split_part(name, '/', 1));