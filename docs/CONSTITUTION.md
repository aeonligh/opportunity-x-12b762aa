...
| **C-18** | **Aggregator dependency for sub-institutional organisers.** *Rescoped by R-14; the R-13 framing was wrong.* Domain novelty is not the variable — NELFUND launched on a brand-new domain from a brand-new agency and was comprehensively discoverable through the Presidency, ministries and universities. **Organiser weight is the variable.** Institutions announce what they are responsible for; a student society, small NGO or informal collective has no such obligation and no larger body announcing on its behalf. For that class, aggregators may be the only practical path, which would reintroduce the CR-19 dependency at system level. **Residue scoped but unquantified — see R-15.** |
