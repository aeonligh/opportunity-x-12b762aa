...
| **C-18** | **Aggregator dependency reintroduces CR-19 at system level.** R-13 found a legitimate competition (DCP University Engineering Challenge, ULES/Dangote, N1.5m) published on `ulesarb.org` — a domain that did not exist before the event. Crawling every known institution would not have found it; it became discoverable because aggregators wrote about it. If discovery of ephemeral-domain opportunities depends on Edugist, Legit.ng, MSME Africa or Scholarship Region noticing and publishing in time, the dependency CR-19 forbids has been relocated from the person's social network to the product's source network — not removed. **Requires founder resolution; not resolvable from evidence.** |
