...
function Analytics() {
  ...
  const cards = [
    ...
    {
      label: "Apply Clicks",
      value: data.totals.apply_click ?? 0,
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {cards.map((c) => (
          <div
            key={c.label}
            className="p-4 rounded-2xl border border-border bg-surface/60 backdrop-blur-md"
          >
      ...
      <h2 className="text-sm font-bold uppercase tracking-wider text-text-s mb-3">
        Top opportunities
      </h2>
        ...
        {Object.entries(data.perOpp)
          .map(([id, totals]) => {
            ...
            return {
              ...
              total:
                (t.view ?? 0) + (t.save ?? 0) * 2 + (t.share ?? 0) * 3 + (t.apply_click ?? 0) * 5,
            };
          })
          ...
          .map((row) => (
            <div
              key={row.id}
              className="flex items-center gap-3 py-2 border-b border-border/50 font-mono"
            >
